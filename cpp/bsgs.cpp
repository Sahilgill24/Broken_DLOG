...
// extern "C"
// {
// #include "sha256.h" // Include the SHA-256 header file
// }
...
void bsgs(std::unordered_map<std::string, int> &lookup_table, const long long int m, const mpz_t a, const mpz_t g, const mpz_t p)
{
...
// std::string hasher(const mpz_t value){
//     SHA256_CTX ctx;
//     uint8_t hash[32];

//     sha256_init(&ctx);
//     char *string_to_hash = mpz_get_str(NULL, 10, value);
//     sha256_update(&ctx, (const BYTE *)string_to_hash, strlen(string_to_hash));
...
int main()
{
    ...
    // g_str = "2";
    // p_str = "10957";
    const long long int m = std::stoll(m_str);
    mpz_t a, g, p;
